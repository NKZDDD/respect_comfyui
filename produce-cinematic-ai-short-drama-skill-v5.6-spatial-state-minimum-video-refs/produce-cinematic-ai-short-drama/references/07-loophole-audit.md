...
修正：在现有Manifest和Prompt中加入六字段`Compact Reference Identity Map`：Exact ID、Who / What + Visible Content、Story Time / Current State、Controls、Does Not Control、Applicable Scope。映射缺失、错序或与Manifest不一致时输出`REFERENCE_MAPPING_BLOCKED`；不新增资产类型或第二套注册表。
...
修正：读取[空间状态门控与视频参考最小化](14-spatial-state-gating-and-video-reference-minimization.md)；锁定World Position State与Seat/Support Binding。相邻状态只允许Authorized Movers按Release→Route→Portal→Target Anchor→Completion换位；没有事件则精确继承。
...
修正：Video默认只上传Canonical Storyboard执行图。SCSTATE在故事板阶段已经完成任务，不重复占槽；人物LOOK/CT、Prop或Location只针对可证明的Coverage/Identity/关键细节缺口补充。

### 7.15 Video参考图装满上限

失败：平台允许10张就上传9至10张，多个身份、空间、道具与状态Authority互相竞争。

修正：执行Minimum Sufficient Reference Set。通常为1至2张Storyboard；超过5张时逐张写`Unique Missing Authority`，无法证明的删除。Clean LOOK与未来CT未经Reference Time Scope验证不得同时全时段上传。
...
- [ ] 身份映射缺失或上传错序会触发`REFERENCE_MAPPING_BLOCKED`，模型不会猜图继续。
...
- [ ] Video默认只上传Storyboard执行图；同一状态SCSTATE未与Storyboard重复作为Composite Authority。
- [ ] 每张Video补充Reference都有Unique Missing Authority；超过5张已完成冲突审计，Clean LOOK与未来CT没有无边界并列。
...
21. 任一Image槽没有明确Who/What、当前Story Time/State、Controls、Does Not Control或Applicable Scope，或映射与上传顺序不一致。
22. 人物Position State改变但没有Authorized Movement Event、合法Route、Portal Crossing或足够完成时间。
23. SCSTATE或Storyboard为同框/构图擅自解除Seat/Support Binding或把人物移动到另一Zone。
24. Video同时把Storyboard与同一状态SCSTATE作为组合主权威，或参考图超过5张却无法逐张证明Unique Missing Authority。
