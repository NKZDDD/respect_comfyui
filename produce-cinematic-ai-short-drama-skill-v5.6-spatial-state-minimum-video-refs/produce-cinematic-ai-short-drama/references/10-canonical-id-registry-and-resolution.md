...
Who / What + Visible Content: {这是谁/是什么，以及图中实际可见内容}
Story Time / Current State: {年龄/PH/LOOK/CT/事件前后/Thread}
Controls: {本次有权控制的维度}
Does Not Control: {本次无权控制的维度}
Applicable Scope: {Target/KF/Time Window}
```

完整ID解决“具体是哪一份文件”，身份映射解决“模型应把图中的谁/什么理解成什么”。二者缺一不可。身份映射直接复用Registry与Manifest信息，不产生新ID、新资产或第二套Registry。

若Exact Resolution已通过，但任一Image缺少主体、当前状态或适用范围，或上传顺序与Prompt映射不一致，返回`REFERENCE_MAPPING_BLOCKED`。
...
Compact Identity Map completeness: PASS
Upload Order ↔ Identity Map: PASS
...
Source CVS、人物Root、Location PR、Prop SPEC/INSTANCE CT及文件Reference全部写完整Revision ID。SCSTATE Prompt逐Image回显Who/What、当前Story Time/State与Scope。SCSTATE本身也先注册为RESERVED，确认后再成为CANONICAL。

### Storyboard

`SOURCE SCSTATE / CVS / VT`、每个KF ID、Storyboard Package ID和Reference ID均逐字符完整。内部可以使用`Image 1`方便模型，但必须同一行保留对应完整ID，并说明图中是谁/是什么、当前状态和Applicable KF。

### Video

Entry/Target KF、SBPKG、人物CT、Prop Instance和所有补充Reference使用完整Revision ID。每个Image还必须说明Who/What、Current State与Applicable Time Window。禁止在Video Prompt里把上游ID缩成`KF03`、`CT01`或“上一板”。
...
| REFERENCE_MAPPING_BLOCKED | Image身份、当前状态、范围或上传顺序映射不完整 | 补齐六字段紧凑映射后重新编译 |
...
【COMPACT REFERENCE IDENTITY MAP】
Image 1 = {exact_reference_revision_id}
Who / What + Visible Content: {identity_and_visible_content}
Story Time / Current State: {story_time_and_state}
Controls: {authority}
...
16. 每个Image槽必须同时拥有完整ID与六字段语义身份映射；缺失时阻断，不让模型猜。
