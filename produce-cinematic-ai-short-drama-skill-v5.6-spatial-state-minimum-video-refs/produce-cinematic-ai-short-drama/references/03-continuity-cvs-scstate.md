...
人物位置还必须记录`zone_id / support_binding_id / support_relation / current_barrier_side / posture_class / last_authorized_movement_event_id`。完整规则读取[空间状态门控与视频参考最小化](14-spatial-state-gating-and-video-reference-minimization.md)。没有批准的移动事件时，这些字段与XYZ、Anchor、Orientation一起持续继承。
...
characters[]:
  ...
  support_binding_id / support_relation
  barrier_side / posture_class
...
6. COMPACT REFERENCE IDENTITY MAP。
7. REFERENCE ROLE MAP。
8. REFERENCE FIREWALL。
9. CHARACTER STATE CONTRACT。
10. PROP STATE CONTRACT。
11. LOCATION & SPATIAL CONTRACT。
12. WORLD PLACEMENT CONTRACT。
13. BLOCKING & RELATION CONTRACT。
14. PRESENCE / VISIBILITY CONTRACT。
15. TEMPORAL STATE LOCK。
16. MUST PRESERVE STATE LIST。
17. FORBIDDEN STATE LIST。
18. CAMERA / PRESENTATION RULE。
19. OUTPUT FORMAT。

`COMPACT REFERENCE IDENTITY MAP`逐Image写完整Revision ID、Who/What与可见内容、Story Time/Current State、Controls、Does Not Control和Applicable Scope。只写“Image 1控制人物LOOK”但不说明Image 1中的人物是谁、哪个年龄/状态或何时有效，SCSTATE Prompt不得释放。
...
movement event
support release
route / barrier crossing
target anchor / support
completion condition
```

情绪、对白、Camera、照片抛出或人物“开始争吵”都不是换位事件。若上一状态人物坐在桌后，下一状态没有起身、绕桌和到达完成，就必须继续坐在同一Support Anchor；需要同框时允许遮挡或另选观察机位，不得把人物搬到桌前。

SCSTATE Prompt必须写`PREVIOUS SCSTATE / UNCHANGED POSITION STATES / AUTHORIZED MOVERS / FORBIDDEN POSITION DELTA`。位置冲突时CVS正确、SCSTATE作废；若业务确需改变Blocking，建立新CVS Revision并重编下游。
