...
- 在每个有Reference的最终Prompt内部加入六字段`COMPACT REFERENCE IDENTITY MAP`：完整ID、Who/What与可见内容、Story Time/Current State、Controls、Does Not Control、Applicable Scope。不要只写“Image 1控制什么”。
...
Image 1 = {full_reference_revision_id}
Who / What + Visible Content: {who_or_what_and_visible_content}
Story Time / Current State: {story_time_and_current_state}
Controls: {authority_in_this_call}
Does Not Control: {excluded_authority}
Applicable Scope: {target/KF/time_window}
...
若Reference Count大于0而任一ID、状态、文件、路径、Fingerprint或Authority未解析，输出`REFERENCE_RESOLUTION_BLOCKED`和错误码，不输出生产Prompt。若文件解析已通过但身份、Story Time/Current State或Applicable Scope不完整，或Image顺序与Prompt不一致，输出`REFERENCE_MAPPING_BLOCKED`，同样不输出生产Prompt。
...
{逐Image写完整Canonical Revision ID、Who/What与可见内容、Story Time/Current State、Controls、Does Not Control、Applicable Scope}
...
【POSITION STATE INHERITANCE】

Previous SCSTATE：{完整Revision ID或NONE}
Unchanged Position States：{逐人物Zone、Anchor、Seat/Support Binding、Posture Class、Barrier Side}
Authorized Movers：{完整人物ID或NONE}
Authorized Movement Event：{完整Event/VT ID或NONE}
Start / Release / Route / Portal / End / Completion：{完整信息；无移动时写NO POSITION DELTA}
Forbidden Position Delta：不得为同框、可见性或情绪升级移动人物。
...
{逐Image写完整Canonical Revision ID、Who/What与可见内容、Story Time/Current State、Controls、Does Not Control、Applicable KF}
...
每个KF写完整Spatial Revision、LOC_VIEW或GEO_PROXY ID、Camera XYZ/Look-at、View Coverage Status，以及人物/Prop World XYZ、Anchor Offset、Orientation、Zone、Seat/Support Binding、Barrier Side、Position Delta和Authorized Movement Event ID。所有视角必须来自同一Geometry Proxy；Camera只投影，不得静默移动门窗、家具、人物或道具。

没有Authorized Movement Event时写`POSITION DELTA = NONE`，精确继承上一KF；不得为了三人同框或争吵构图把坐在桌后的人搬到前场。
...
【VIDEO REFERENCE MINIMUM SUFFICIENT SET】

Primary Composite Authority：{Canonical Storyboard执行图1...N}
Removed Duplicate Composite References：同一状态SCSTATE不得与Storyboard同时上传。
Supplemental References：{只列经Coverage/Identity/关键文字缺口证明后加入的LOOK/CT或Hero Prop；没有则写NONE}
Reference Count Rationale：{逐张写Unique Missing Authority；超过5张必须先执行冲突审计}

若Storyboard已经清楚包含人物、Location、Prop与Blocking，不再上传相同SCSTATE、PR、Location View或Atomic Prop。任一Supplemental Reference不得控制Camera、Pose、Blocking、Action Phase或Time。

【COMPACT REFERENCE IDENTITY MAP】

{逐Image写完整Canonical Revision ID、Who/What与可见内容、Story Time/Current State、Controls、Does Not Control、Applicable Time Window}
...
{逐人物写Zone、Anchor、Seat/Support Binding、Posture Class、Barrier Side与上一合法Movement Event。没有Authorized Movement Event时全段继承；任何换位写Release、Route、Portal Crossing、Target Anchor和Completion。}
...
禁止人物换脸、年龄漂移、服装重设、无事件离开座椅/支撑或跨Zone瞬移、为构图重新Blocking、Storyboard与同状态SCSTATE双主权、自由想象未定义身体/背面/下装/鞋履、伤口错位/消失、未来状态提前、PROP SPEC漂移、INSTANCE交换/克隆/丢失、Holder错位、销毁后完整重生、空间镜像、超出Geometry Proxy覆盖、额外人物、Panel Morph、Shot Morph、静态滑动、融化变形、多个分段素材输出、外部剪辑依赖和未授权剧情变化。
