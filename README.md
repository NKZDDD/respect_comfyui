...
把**多个中转 / 直连 API 网关**封装成 ComfyUI 节点（小裴 aicopy、一花 Codex、坤鸡、章鱼哥、零视工坊、灵感鸭、鹤 paisio、M86、一手 ONE API、阿珂、超模、小霸龙、好漫剧、巨轮、Gate、无限画布），
...
| **无限画布** | `https://videogogo.top/api` | — | **无限画布 视频**（Seedance 2.5，本机图自动传 asset） | — |
...
| 无限画布 / videogogo | **不是阿珂。** 只做视频，**分辨率由模型名决定**（`-720p` → 720p，`seedance-2.5-hf` → 480p），没有单独的分辨率参数。本机图走 **`POST /v1/assets`**（裸字节 + `X-File-Name` 头）换 `asset_id`，所以 IMAGE 口能直接用、不用先配对象存储。提交带 **`Idempotency-Key`**，重投不重复计费 |
